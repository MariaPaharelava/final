...
public class UserDAO extends AbstractDAO<User> {
    private static final String SQL_SEARCH_ALL_USER = "SELECT `users_id`, `login`, `users_password`, `fname`, `lname`, `users_role`, `is_blocked` FROM `users`;";
    private static final String SQL_SEARCH_USER_BY_ID = "SELECT `users_id`, `login`, `users_password`, `fname`, `lname`, `users_role`, `is_blocked` FROM `users` WHERE `users_id` = ?;";
    ...
    private static final String SQL_UPDATE_USER_BY_ID = "UPDATE `users` SET `users_id` = ?, `login` = ?, `users_password` = ?, `fname` = ?, `lname` = ?, `users_role` = ? , `is_blocked` = ? WHERE  `users_id` = ?;";
    private static final String SQL_SEARCH_USER_BY_LOGIN = "SELECT `users_id`, `login`, `users_password`, `fname`, `lname`, `users_role`,`is_blocked` FROM `users`  WHERE `login` = ?;";
    ...
    public void save(User entity) throws DAOException {
        ...
        if (entity.getID() == null) {
            ...
            executeUpdate(SQL_UPDATE_USER_BY_ID, entity.getID(), entity.getLogin(), entity.getPassword(),
                    entity.getSurname(), entity.getName(), role, entity.getBlocked(), entity.getID());
        }
    }
}
